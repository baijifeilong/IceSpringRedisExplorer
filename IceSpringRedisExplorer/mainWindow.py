...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _setupView(self):
        ...
        self._treeView.setEditTriggers(gg(QtWidgets.QTreeView.NoEditTriggers))
        ...
        self._detailSplitter = QtWidgets.QSplitter(gg(QtCore.Qt.Vertical))
        ...
        self._infoEdit = QtWidgets.QTextEdit("Info")
        self._keyEdit = QtWidgets.QTextEdit("Key")
        self._valueWidget = QtWidgets.QWidget()
        ...
        self._valueEdit = QtWidgets.QTextEdit("Value")
        self._valueRadioLayout = QtWidgets.QHBoxLayout()
        self._valueAutoRadio = QtWidgets.QRadioButton("Auto")
        self._valueAutoRadio.setChecked(True)
        self._valueRawRadio = QtWidgets.QRadioButton("Raw")
        self._valueJsonRadio = QtWidgets.QRadioButton("JSON")
        self._valueRadioGroup = QtWidgets.QButtonGroup()
        ...
        for source in sources:
            ...
            sourceNode.setData(dict(type="SOURCE", host="localhost", port=6379, password=None), gg(QtCore.Qt.UserRole))
    ...
    def _onTreeViewDoubleClicked(self, modelIndex: QtCore.QModelIndex):
        ...
        if jd["type"] == "SOURCE" and not clickedNode.hasChildren():
            for key in rds.info("KEYSPACE"):
                ...
                childNode.setData(dict(type="SCHEMA", db=db), gg(QtCore.Qt.UserRole))
        ...
        if jd["type"] == "SCHEMA" and not clickedNode.hasChildren():
            ...
            self._treeView.sortByColumn(0, gg(QtCore.Qt.AscendingOrder))
    ...
    def _refreshValue(self, ):
        ...
        _type = self._valueRadioGroup.checkedButton().text()
        _type = _type if _type != "Auto" else self._detectTextFormat(value.decode())
        text = json.dumps(self._loadJson(value), indent=4, ensure_ascii=False) if _type == "JSON" else value.decode()
